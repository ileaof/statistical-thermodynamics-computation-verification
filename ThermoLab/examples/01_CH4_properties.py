"""Example 01 — Pure-fluid (CH4) properties from a (T, P) specification.

This is the canonical quick-start from the ThermoLab specification::

    from thermolab import Gas
    ch4 = Gas("CH4", backend="thermopack")
    st = ch4.state(T=800.0, P=5e5)

Run with::

    python examples/01_CH4_properties.py
"""
...
def main() -> None:
    ...
    print(f"CH4 at T = {st.T:.1f} K, P = {st.P:.3e} Pa")
    print("-" * 52)
    for attr in (
    ...
    st2 = ch4.state(P=5e5, h=st.h)  # recover the same state from (P, h)
