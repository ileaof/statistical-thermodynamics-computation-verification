"""Example 01 — Pure-fluid (CO2) properties from a (T, P) specification.

This is the canonical quick-start from the ThermoLab specification::

    from thermolab import Gas
    co2 = Gas("CO2", backend="thermopack")
    st = co2.state(T=800.0, P=5e5)

Run with::

    python examples/01_CO2_properties.py
"""
...
def main() -> None:
    ...
    print(f"CO2 at T = {st.T:.1f} K, P = {st.P:.3e} Pa")
    print("-" * 52)
    for attr in (
    ...
    st2 = co2.state(P=5e5, h=st.h)  # recover the same state from (P, h)
